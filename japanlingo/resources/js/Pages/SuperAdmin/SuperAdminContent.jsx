...
export default function SuperAdminContent({
    ...
    const deleteAttachment = (attachmentId) => {
        ...
        router.delete(route('superadmin.content.news.attachments.destroy', { news: editingNews.id, attachment: attachmentId }), {
            preserveScroll: true,
                                ...
                                {updates.map((item, index) => (
                                    <div key={item.id || `${item.item}-${item.by}-${item.created_at || index}`} className="rounded-2xl border border-gray-100 dark:border-gray-800 p-4">
                                        ...
                                        <div className="mt-2 flex flex-wrap gap-2">
                                            ...
                                            <span className="rounded-full bg-blue-50 dark:bg-blue-900/20 px-3 py-1 text-xs font-black text-blue-700 dark:text-blue-400">{item.state}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                                                ...
                                                {selectedNewsAttachments.map((attachment) => (
                                                    <div key={attachment.id} className="rounded-xl border border-gray-100 dark:border-gray-800 p-3 text-sm">
                                                        <div className="flex items-center justify-between gap-3">
                                                            ...
                                                            <button type="button" onClick={() => deleteAttachment(attachment.id)} className="text-xs font-black text-red-600 dark:text-red-400">Hapus</button>
                                                        </div>
                                                        {attachment.url && <a href={attachment.url} target="_blank" rel="noreferrer" className="mt-2 inline-block text-xs font-bold text-blue-600 dark:text-blue-400">Buka file</a>}
                                                        {attachment.video_embed_url && <a href={attachment.video_embed_url} target="_blank" rel="noreferrer" className="mt-2 inline-block text-xs font-bold text-blue-600 dark:text-blue-400">Buka video</a>}
                                                    </div>
                                                ))}
                                            </div>
            ...
            {deleteTarget && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="w-full max-w-md rounded-2xl bg-white dark:bg-gray-900 p-6 shadow-2xl">
                        <h3 className="text-lg font-black text-gray-900 dark:text-white">Hapus News</h3>
                        <p className="mt-2 text-sm font-medium text-gray-500 dark:text-gray-400">{deleteTarget.title} akan dihapus dari portal berita.</p>
                        <div className="mt-6 flex justify-end gap-3">
                            <button onClick={() => setDeleteTarget(null)} className="rounded-xl border border-gray-200 dark:border-gray-700 px-5 py-2.5 text-sm font-bold text-gray-600 dark:text-gray-300">Batal</button>
                            <button onClick={deleteNews} className="rounded-xl bg-red-600 px-5 py-2.5 text-sm font-black text-white">Hapus</button>
                        </div>
                    </div>
                </div>
            )}
