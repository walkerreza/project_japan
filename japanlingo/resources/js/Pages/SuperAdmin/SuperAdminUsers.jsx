                ...
                <div className="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {['Semua', 'Aktif', 'Review', 'Suspended'].map((item, index) => (
                            <span key={item} className={`rounded-full px-4 py-2 text-sm font-bold ${index === 0 ? 'bg-red-600 text-white' : 'border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-600 dark:text-gray-400'}`}>
                                {item}
                            </span>
                        ))}
                    </div>
                ...
                <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.2fr_0.8fr]">
                    <Card padding={false}>
                        <div className="border-b border-gray-100 dark:border-gray-800 px-6 py-4">
                            ...
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Snapshot akun yang paling butuh perhatian operasional.</p>
                        </div>
                        <div className="overflow-x-auto">
                            <table className="min-w-[760px] w-full text-sm">
                                ...
                                <tbody>
                                    {users.map((item) => (
                                        <tr key={item.email} className="border-t border-gray-100 dark:border-gray-800">
                                            <td className="px-6 py-4">
                                                <div className="font-bold text-gray-900 dark:text-white">{item.name}</div>
                                                <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">{item.email}</div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <span className={`rounded-full px-3 py-1 text-xs font-black ${item.status === 'Aktif' ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400' : item.status === 'Review' ? 'bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400' : 'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400'}`}>
                                                    {item.status}
                                                </span>
                                            </td>
                    ...
                    <div className="space-y-6">
                        <Card>
                            <h2 className="text-lg font-black text-gray-900 dark:text-white">Aksi Cepat</h2>
                        ...
                        <Card>
                            <h2 className="text-lg font-black text-gray-900 dark:text-white">Antrian Review</h2>
