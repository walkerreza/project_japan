...
export default function BerandaUser({ user = {}, recentProgress = [], availableLevels = [], rewardHistory = [], news = [], activeSubscription = null }) {
    const accessKeyForm = useForm({ code: '' });

    const submitAccessKey = (event) => {
        event.preventDefault();
        accessKeyForm.post(route('user.access-keys.redeem'), {
            preserveScroll: true,
            ...
            <div className="w-full bg-white dark:bg-gray-950 min-h-screen pb-16 transition-colors duration-300">
                ...
                >
                    ...
                    <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center flex flex-col items-center">
                        ...
                        {false && user.subscription_status !== 'premium' && (
                        ...
                        {user.subscription_status !== 'premium' && (
                            <div className="mb-8 w-full max-w-xl rounded-3xl border border-white/70 bg-white/85 p-4 text-left shadow-xl shadow-red-900/5 backdrop-blur dark:border-gray-800 dark:bg-gray-900/85">
                                <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                                    <div>
                                        <p className="text-[10px] font-black uppercase tracking-[0.22em] text-red-500">Access Key Premium</p>
                                        <p className="mt-1 text-sm font-bold text-gray-700 dark:text-gray-300">Masukkan kode dari sensei untuk membuka materi premium.</p>
                                    </div>
                                    <Link href={route('pricing')} className="text-xs font-black text-red-600 hover:underline">Lihat paket</Link>
                                </div>
                                <form onSubmit={submitAccessKey} className="mt-4 flex flex-col gap-2 sm:flex-row">
                                    <input
                                        ...
                                        className="h-12 flex-1 rounded-2xl border border-gray-200 bg-white px-4 text-sm font-black uppercase tracking-widest text-gray-900 outline-none focus:border-red-300 focus:ring-4 focus:ring-red-100 dark:border-gray-700 dark:bg-gray-950 dark:text-white dark:focus:ring-red-900/30"
                                    />
                                    <button disabled={accessKeyForm.processing} className="h-12 rounded-2xl bg-red-600 px-5 text-sm font-black text-white shadow-lg shadow-red-500/20 transition-colors hover:bg-red-700 disabled:opacity-50">
                                        {accessKeyForm.processing ? 'Mengecek...' : 'Redeem'}
                                    </button>
                                </form>
                                {accessKeyForm.errors.access_key && <p className="mt-2 text-xs font-bold text-red-600">{accessKeyForm.errors.access_key}</p>}
                            </div>
                        )}

                        {user.subscription_status === 'premium' && activeSubscription && (
                            <div className="mb-8 rounded-full border border-yellow-200 bg-yellow-50 px-5 py-2 text-xs font-black text-yellow-700 dark:border-yellow-900/40 dark:bg-yellow-900/20 dark:text-yellow-300">
