...
export default function LessonEdit({ lesson, modules = [] }) {
    ...
    return (
        <AuthenticatedLayout>
            <Head title={`Edit: ${lesson.title} - Japanlingo`} />
            <div className="min-h-screen bg-[#F8F9FB] font-sans">
                ...
                <header className="bg-white border-b border-gray-200 px-6 py-4 sticky top-0 z-40 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <Link href={route('admin.lessons.index')} className="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center text-gray-400 hover:text-gray-900 transition-colors">
                        ...
                        <div>
                            <h1 className="text-sm font-black text-gray-900">Edit Pelajaran</h1>
                            <p className="text-[11px] text-gray-400 font-medium truncate max-w-xs">{lesson.title}</p>
                        </div>
                    </div>
                    <button
                ...
                <main className="max-w-3xl mx-auto p-6 space-y-5">
                    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-4">
                        <h2 className="text-sm font-black text-gray-700 uppercase tracking-wider">Informasi Pelajaran</h2>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="col-span-2">
                                <label className="block text-sm font-bold text-gray-700 mb-1.5">Modul <span className="text-red-500">*</span></label>
                                <select
                            ...
                            <div className="col-span-2">
                                <label className="block text-sm font-bold text-gray-700 mb-1.5">Judul Pelajaran <span className="text-red-500">*</span></label>
                                <input
                                    type="text"
                                    value={data.title}
                            ...
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1.5">Durasi (menit)</label>
                                <input type="number" min="1" value={data.duration_minutes} onChange={e => setData('duration_minutes', e.target.value)}
                                    className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#E64A19]/30 focus:border-[#E64A19]"
                                    placeholder="15"
                                />
                            </div>
                    ...
                    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-3">
                        <h2 className="text-sm font-black text-gray-700 uppercase tracking-wider">Konten Materi</h2>
