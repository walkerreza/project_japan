...
export default function ModulesIndex({ modules, levels = [], filters = {} }) {
    ...
    const levelColors = { N5: 'bg-green-100 text-green-700', N4: 'bg-blue-100 text-blue-700', N3: 'bg-yellow-100 text-yellow-700', N2: 'bg-orange-100 text-orange-700', N1: 'bg-red-100 text-red-700' };
            ...
            <div className="min-h-screen bg-[#F8F9FB] font-sans">
                ...
                <header className="bg-white border-b border-gray-200 px-6 py-4 sticky top-0 z-40 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-sm font-black text-gray-900">Manajemen Modul & Materi</h1>
                            <p className="text-[11px] text-gray-400 font-medium">{modules.length} Modul tersedia</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                ...
                <main className="max-w-5xl mx-auto p-6 space-y-4">
                    {filteredModules.map(module => (
                        <div key={module.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                            <div className="flex items-center gap-4 p-5">
                                <button onClick={() => toggleExpand(module.id)} className="text-gray-400 hover:text-gray-700 transition-colors">
                                    {expandedModules[module.id] ? <ExpandMoreIcon sx={{ fontSize: 22 }} /> : <ChevronRightIcon sx={{ fontSize: 22 }} />}
                                </button>
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className={`text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-md ${levelColors[module.level?.level_name] || 'bg-gray-100 text-gray-600'}`}>
                                            {module.level?.level_name || '—'}
                                        </span>
                                        <span className="text-[11px] text-gray-400 font-medium">Minggu {module.week_number}</span>
                                    </div>
                                    <h2 className="text-base font-black text-gray-900 truncate">{module.title}</h2>
                                </div>
                                <div className="flex items-center gap-4 shrink-0">
                                    ...
                                    <button onClick={() => openEditModal(module)} className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-blue-600 hover:bg-blue-50">
                                        <EditOutlinedIcon sx={{ fontSize: 18 }} />
                                    </button>
                                    <button onClick={() => handleDelete(module)} className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-red-600 hover:bg-red-50">
                            ...
                            {expandedModules[module.id] && (
                                <div className="border-t border-gray-100 bg-gray-50/50">
                                    <div className="px-5 py-3 flex items-center justify-between">
                                        <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Daftar Pelajaran</p>
                                        <Link
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1.5">Level JLPT <span className="text-red-500">*</span></label>
                                <select
                                    value={data.level_id}
                            ...
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1.5">Judul Modul <span className="text-red-500">*</span></label>
                                <input
                                    type="text"
                                    value={data.title}
                            ...
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1.5">Nomor Minggu <span className="text-red-500">*</span></label>
                                <input
                                    type="number"
                                    min="1"
                            ...
                            <div className="flex gap-3 pt-2">
                                <button type="button" onClick={() => setShowModuleModal(false)} className="flex-1 h-11 rounded-xl border border-gray-200 text-sm font-bold text-gray-600">Batal</button>
            ...
            {deleteConfirm && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
                        ...
                        <h3 className="text-base font-black text-gray-900 mb-2">Hapus Modul?</h3>
                        <p className="text-sm text-gray-500 mb-6">Apakah Anda yakin ingin menghapus modul <strong>"{deleteConfirm.title}"</strong>?</p>
                        <div className="flex gap-3">
                            <button onClick={() => setDeleteConfirm(null)} className="flex-1 h-11 rounded-xl border border-gray-200 text-sm font-bold text-gray-600">Batal</button>
