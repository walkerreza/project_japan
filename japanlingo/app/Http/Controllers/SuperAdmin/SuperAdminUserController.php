    ...
    public function __invoke(Request $request)
    {
        ...
        return Inertia::render('SuperAdmin/DataUser/DataUser', [
            'stats' => [
                ...
                $this->stat('Perlu Review', number_format(User::where('role', 'user')->whereNull('last_activity_date')->count()), 'R', '0', 'down'),
                $this->stat('Akun Suspended', number_format(User::where('role', 'user')->where('status', 'suspended')->count()), 'S', '0', 'down'),
            ],
            'users' => $students->through(fn (User $user) => [
                'id' => $user->id,
                'name' => $user->username,
                'email' => $user->email,
