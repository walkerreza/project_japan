...
export default function ManajemenBoard({ boards = {}, filters = {}, levels = [], modules = [], lessons = [] }) {
    ...
    const submitForm = (event) => {
        ...
        return editing
            ? form.put(route('admin.boards.update', editing.id), { preserveScroll: true, onSuccess: closeForm })
            : form.post(route('admin.boards.store'), { preserveScroll: true });
    };

    const submitFilters = (event) => {
        event.preventDefault();
        router.get(route('admin.boards.index'), { search, status }, { preserveState: true, replace: true });
    };

    const deleteBoard = (board) => {
        if (!window.confirm(`Hapus board "${board.title}"?`)) return;
        router.delete(route('admin.boards.destroy', board.id), { preserveScroll: true });
    };

    return (
        <AuthenticatedLayout>
            <Head title="Admin - Board Ajar" />

            <div className="space-y-6 px-4 py-6 sm:px-6 lg:px-8">
                <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
                    <div>
                        <p className="text-xs font-black uppercase tracking-[0.3em] text-orange-600">Teaching Board</p>
                        <h1 className="text-2xl font-black text-gray-900 dark:text-white">Board Ajar</h1>
                        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Papan tulis digital untuk corat-coret materi sensei.</p>
                    </div>
                ...
                <Card>
                    <form onSubmit={submitFilters} className="grid grid-cols-1 gap-3 md:grid-cols-[1fr_180px_auto]">
                        <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Cari board..." className="h-11 rounded-xl border border-gray-200 bg-white px-4 text-sm text-gray-900 dark:border-gray-700 dark:bg-gray-950 dark:text-white" />
                            ...
                            <div className="flex items-start justify-between gap-4">
                                <div>
                                    <div className="flex flex-wrap gap-2">
                                        <span className={`rounded-full px-2 py-1 text-[10px] font-black ${board.status === 'published' ? 'bg-green-50 text-green-600 dark:bg-green-900/20' : 'bg-yellow-50 text-yellow-600 dark:bg-yellow-900/20'}`}>{board.status}</span>
                                        {board.lesson && <span className="rounded-full bg-orange-50 px-2 py-1 text-[10px] font-black text-orange-600 dark:bg-orange-900/20">Lesson</span>}
                                    </div>
                                    <h2 className="mt-3 text-lg font-black text-gray-900 dark:text-white">{board.title}</h2>
                                    <p className="mt-2 line-clamp-2 text-sm text-gray-500 dark:text-gray-400">{board.description || 'Belum ada deskripsi.'}</p>
                                    <p className="mt-3 text-xs font-bold text-gray-400">{board.level?.level_name || 'General'} {board.module ? `- ${board.module.title}` : ''}</p>
                                </div>
                            </div>
                            <div className="mt-5 grid grid-cols-2 gap-2">
                                <Link href={route('admin.boards.editor', board.id)} className="rounded-xl bg-orange-600 px-4 py-2 text-center text-xs font-black text-white">Editor</Link>
                                <button onClick={() => openEdit(board)} className="rounded-xl border border-gray-200 px-4 py-2 text-xs font-black text-gray-700 dark:border-gray-700 dark:text-gray-200">Edit</button>
                                <button onClick={() => deleteBoard(board)} className="col-span-2 rounded-xl border border-red-100 px-4 py-2 text-xs font-black text-red-600 dark:border-red-900/40">Hapus</button>
                            </div>
                        </Card>
                    ))}
                </div>

                {rows.length === 0 && <Card><p className="text-center text-sm font-bold text-gray-500">Belum ada board ajar.</p></Card>}
                        ...
                        <div className="mx-auto my-8 max-w-3xl rounded-3xl bg-white p-6 shadow-2xl dark:bg-gray-900">
                            <div className="mb-6 flex items-start justify-between gap-4">
                                <div>
                                    ...
                                    <h2 className="text-xl font-black text-gray-900 dark:text-white">{editing ? 'Edit Board' : 'Buat Board Baru'}</h2>
                                </div>
                            ...
                            <form onSubmit={submitForm} className="space-y-4">
                                <input value={form.data.title} onChange={(event) => form.setData('title', event.target.value)} placeholder="Judul board" className="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm dark:border-gray-700 dark:bg-gray-950 dark:text-white" />
