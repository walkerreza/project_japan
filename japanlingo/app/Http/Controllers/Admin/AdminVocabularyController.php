...
class AdminVocabularyController extends Controller
{
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($query) use ($search) {
                $query->where('word', 'ilike', "%{$search}%")
                    ...
                    ->orWhere('meaning_en', 'ilike', "%{$search}%")
    ...
    {
        Vocabulary::create($this->validateVocabulary($request));
    ...
    public function import(Request $request)
    {
        ...
        if (! in_array($extension, ['csv', 'txt'], true)) {
            return redirect()->back()->withErrors(['import_file' => 'Gunakan CSV untuk import Vocabulary Bank.']);
        }
    ...
    private function validateVocabulary(Request $request, ?Vocabulary $vocabulary = null): array
    {
